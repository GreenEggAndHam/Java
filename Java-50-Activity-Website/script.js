...
async function init(){
  ...
  let link = "https://cautious-waffle-q74qjv9p76vqcxxrq-8500.app.github.dev"; 
  // The URL above must be running in a separate broswer tab. (Alternate use: http://localhost:8500 )
  let route= "/songs";

  info = await fetch(link+route);
  songs = await info.json();
  console.log(songs); // confirm data retrieval, makes sure that the webpage can pull the data
