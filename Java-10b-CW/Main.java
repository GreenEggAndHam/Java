class Main {
  ...
  */
  String collegeApp(double gpa, int sat){
    if(gpa>= 3.2 || sat>=1450)
      return "ACCEPTED!! Congrats bro";
    else
      return "Not Accepted, sorry";
  }
  ...
  */
  String ecoFuel(double speed){
      if (speed>=40 && speed<=65)
        return "Fuel Economy, sireeee";
      else
        return "Not Optimal, tsk tsk tsk";
    }
  ...
  */
  ...
  double discount(double item1, double itm2,double itm3 ){
    double totalcost = item1+itm2+itm3;
    
    if(totalcost >250 && (item1>=100||itm2>=100||itm3>=100))
      return totalcost*.90;
    else
    return totalcost;
  }
