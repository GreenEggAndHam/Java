 ...
 public static void main(String[] args)throws IOException{
  ...
  void init() throws IOException{   
    ...
    */
    ...
    String sql = "";
    sql = " Select tracks.Name,albums.title,tracks.composer FROM tracks, albums WHERE albums.albumid=tracks.albumid LIMIT 100;";   
  ...
  //  squeal = "Select * from employees;";
  //  server.createContext("/employees", new RouteHandler(db,squeal));

  //  squeal = "Select * from customers;";
  //  server.createContext("/customers", new RouteHandler(db,squeal));

  //  squeal = "Select * from tracks, albums, artists WHERE albums.albumId=tracks.albumId AND albums.artistId=artist.artistid LIMIT 15;";
  //  server.createContext("/albumsinfo", new RouteHandler(db,squeal));

  //  squeal = "Select customers.firstname, customers.lastname, tracks.name, invoices.invoicedate from tracks, invoice_items, invoices, customers WHERE invoice_items.trackid=tracks.trackId AND invoices.invoiceid=invoice_items.invoiceid AND invoices.customerid=customers.customerid;";
  //   server.createContext("/songer", new RouteHandler(db,squeal));
